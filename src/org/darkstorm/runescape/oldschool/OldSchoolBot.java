...
public class OldSchoolBot implements Bot, EventListener {
	...
	// private final Map<Object, Object> modelCache = new ConcurrentHashMap<>();
	...
	public OldSchoolBot(DarkBot darkbot) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				Cache cache = new EmptyCache(); // new DirectoryCache(new File("cache"));
				int world = 1 + (int) (Math.random() * 78);
				if(cache.isCached("world")) {
					try {
						world = Integer.parseInt(new String(cache.loadCache("world")).replace("\n", "").trim());
				...
				try {
					...
					System.out.println(game.getClass().getField("bot").get(null));
	...
	public void onCallback(CallbackEvent event) {
		...
		}// else if(event.getCallback().equals("model")) {
			// if(modelCache.size() > 500)
		// modelCache.clear();
		// modelCache.put(event.getArguments()[0], event.getArguments()[1]);
		// }
	}

	// public Object getModel(Object animable) {
	// return modelCache.get(animable);
	// }
	//
	// public void setModel(Object animable, Object model) {
	// modelCache.put(animable, model);
	// }
