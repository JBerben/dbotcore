...
public class OldSchoolLoader implements Loader, AppletStub, AppletContext {
	public static final String HOOKS_PACKAGE = OldSchoolLoader.class.getPackage().getName() + ".hooks.I";
	...
	public void load(Cache cache, Status status) throws IOException {
		...
		if(!cache.isCached("launcher")) {
			...
			data = download(new URL("http://oldschool" + world + ".runescape.com/" + jarName), status);
			...
			/*updater.registerTransformer(new Transformer(updater) {
				{
					// HUEHUEHUEHUEHUEHUE
					// ...nothing going on here...
					HookLoader loader = new VInsertJSONHookLoader(updater
							.getClasses(), HOOKS_PACKAGE, Mouse.class
							.getPackage().getName() + ".");
					loader.load();
					hooks.addAll(Arrays.asList(loader.getHooks()));
		...
		if(!cache.isCached("euclid_pairs")) {
			EuclideanNumberDeobber numberDeobber = new EuclideanNumberDeobber(injector);
			...
			String[] euclideanPairsUnparsed = new String(cache.loadCache("euclid_pairs")).split("\n");
			for(String line : euclideanPairsUnparsed) {
				...
				try {
					...
					EuclideanNumberPair pair = new EuclideanNumberPair(product, quotient, gcd, bits, unsafe);
					pairs.put(key, pair);
		...
		classLoader = new CustomClassLoader(new ProtectionDomain(new CodeSource(new URL("http://runescape.com/"), (CodeSigner[]) null), getPermissions()));
		int progress = 0;
		for(ClassGen classGen : newClasses) {
			classLoader.addClass(classGen.getClassName(), classGen.getJavaClass().getBytes());
			status.setProgress((int) ((++progress / (double) newClasses.size()) * 100));
			status.setMessage(message + " - " + progress + "/" + newClasses.size() + " (" + status.getProgress() + "%)");
		}
		classLoader.addURL(new File("../DarkBotRS-OldSchoolImpl/bin/").toURI().toURL());
	}
	...
	private String loadPage(Cache cache) throws IOException {
		...
		if(!cache.isCached("parameters")) {
			URL worldURL = new URL("http://oldschool" + world + ".runescape.com/j1");
			BufferedReader reader = new BufferedReader(new InputStreamReader(worldURL.openStream()));
			String line;
			...
			while((line = reader.readLine()) != null) {
				if(line.contains("<param name=")) {
					...
					cacheText.append("key=\"").append(key).append("\",value=\"").append(value).append("\"\n");
					logger.finer("Found applet parameter, key: " + key + ", value: " + value);
				} else if(line.contains("archive=gamepack_")) {
					appletArchive = line.split("archive=")[1].split(" ")[0];
					cacheText.append("archive=\"").append(appletArchive).append("\"\n");
			...
			String[] lines = new String(cache.loadCache("parameters")).split("\n");
	...
	public GameContext createContext() {
		try {
			Class<?> c = classLoader.loadClass("org.darkstorm.runescape.oldschool.impl.GameContextImpl");
	...
	public static void main(String[] args) throws ClassNotFoundException, IOException {
		HookLoader loader = new VInsertJSONHookLoader(null, "org.darkstorm.runescape.oldschool.hooks.", "org.darkstorm.runescape.oldschool.overrides.");
		loader.load();
		...
		for(Hook hook : hooks)
			if(!(hook instanceof InterfaceHook))
				interfaceElements.get(hook.getInterfaceName()).addContent(hook.toXML());
	...
	private static class AudioClipSub implements AudioClip {
		public static final short STATE_STOPPED = 0, STATE_PLAYING = 1, STATE_LOOPING = 2;
