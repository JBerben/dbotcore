...
public class EuclideanNumberDeobber extends Deobber {
	...
	public void deob(ClassGen classGen) {
		String[] patterns = {
				"((ldc|ldc_w) (getstatic|getfield) imul)|(ldc2_w (getstatic|getfield) lmul)",
		...
		for(Method m : classGen.getMethods()) {
			...
			for(int i = 0; i < patterns.length; i++) {
				...
				Iterator<InstructionHandle[]> iterator = instructionFinder
						.search(patterns[i]); iterator.hasNext();) {
					InstructionHandle[] ih = iterator.next();
					FieldInstruction fi = (FieldInstruction) ih[fieldIndices[i]]
							.getInstruction();
					String descriptor = fi.getReferenceType(cpg).toString()
							+ "." + fi.getFieldName(cpg);
					Number value;
					...
					if(ih[ldcIndex].getInstruction() instanceof LDC) {
						if(((LDC) ih[ldcIndex].getInstruction()).getType(cpg) != Type.INT)
							continue;
						value = (Integer) ((LDC) ih[ldcIndex].getInstruction())
								.getValue(cpg);
					} else if(ih[ldcIndex].getInstruction() instanceof LDC2_W) {
						if(((LDC2_W) ih[ldcIndex].getInstruction())
								.getType(cpg) != Type.LONG)
							continue;
						value = ((LDC2_W) ih[ldcIndex].getInstruction())
								.getValue(cpg);
					} else
						continue;
					if((value.longValue() & 1) == 0)
						continue;
					List<EuclideanNumberPair> values = this.values
							.get(descriptor);
					if(values == null) {
					...
					if(fi instanceof GETFIELD || fi instanceof GETSTATIC)
						values.add(decipherP(
								BigInteger.valueOf(value.longValue()),
								value instanceof Long ? 64 : 32,
								new AtomicBoolean(false)));
					else
						values.add(decipherQ(
								BigInteger.valueOf(value.longValue()),
								value instanceof Long ? 64 : 32,
								new AtomicBoolean(false)));
				}
		...
		try {
			...
			for(String key : values.keySet()) {
				...
				if(nonDuplicates.size() > 1) {
					...
					writer.write("# More than one value for " + key + ": "
							+ nonDuplicates);
					writer.newLine();
					int highestCount = 0;
					...
					if(highestValues.size() > 1) {
						writer.write("# More than one value with the same occurence for "
								+ key + ": " + highestValues);
						writer.newLine();
						value = highestValues;
					} else {
						writer.write("# Resolved conflict for " + key + ": "
								+ highestValues.get(0));
						writer.newLine();
	...
	private EuclideanNumberPair decipherP(BigInteger product, int bits,
	...
	private EuclideanNumberPair decipherQ(BigInteger quotient, int bits,
			AtomicBoolean unsafe) {
		BigInteger product = inverse(quotient, bits);
		return decipher(product, quotient, bits, unsafe);
	}

	private EuclideanNumberPair decipher(BigInteger product,
			BigInteger quotient, int bits, AtomicBoolean unsafe) {
	...
	public static final class EuclideanNumberPair {
		...
		public boolean equals(Object obj) {
			...
			return product.equals(pair.product)
					&& quotient.equals(pair.quotient);
		}
