...
public class InstructionSearcher {
	...
	public InstructionSearcher(ClassGen classGen, Method method) {
		this(new MethodGen(method, classGen.getClassName(),
				classGen.getConstantPool()).getInstructionList(), classGen
				.getConstantPool());
	}
	...
	public InstructionSearcher(InstructionList instructionList,
			ConstantPoolGen cpg) {
		this.cpg = cpg;
		...
		while(getNext() != null)
			try {
				if(ConstantPushInstruction.class.isInstance(getCurrent())) {
					if(((ConstantPushInstruction) getCurrent()).getValue()
							.equals(value))
						return getCurrent();
	...
	@SuppressWarnings("unchecked")
	public <T extends FieldInstruction> T getNextFieldInstruction(
			Class<T> type, String fieldType) {
	...
	@SuppressWarnings("unchecked")
	public <T extends InvokeInstruction> T getNextInvokeInstruction(
			Class<T> type, String methodType) {
		while(getNext(type) != null) {
	...
	public InstructionHandle[][] getPattern(Method method, String pattern) {
		...
		InstructionFinder instructionFinder = new InstructionFinder(
				new InstructionList(method.getCode().getCode()));
		for(Iterator<InstructionHandle[]> it = instructionFinder
				.search(pattern); it.hasNext();) {
			results.add(it.next());
		}
	...
	public void setIndex(int index) {
		...
		if(index >= length || index < 0)
			throw new IndexOutOfBoundsException(
					"param 0 (type int) is out of bounds");
